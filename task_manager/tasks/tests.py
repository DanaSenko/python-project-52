...
class TestTask(TestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(name="Test Task", status=self.status, author=self.user)
    ...
    def test_task_delete_view(self):
        response = self.client.post(reverse("tasks:task_delete", args=[self.task.pk]), follow=True)
        self.assertEqual(response.status_code, 200)
